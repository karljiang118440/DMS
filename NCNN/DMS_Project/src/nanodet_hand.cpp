...
int MTCNNDetection() {
   ...
   //convert to vector and store into fc, whcih is benefical to furthur operation
    ...
    //用来测试视频，完全正常，没有任何问题
    // VideoCapture cap; //using camera capturing
    // cap.open("../img/karl.mp4");
...
/****************************jump*****************************************************/                
                ...
                if (0) {
                    ...
                    cv::waitKey(2000);
                    cv::imwrite(project_path+ format("/img/%d.jpg", count), aligned);
                }
                ...
                sprintf(string, "%.4f", current);
                 ...
                 if (current >= face_thre && y2-y>= distance_threshold){
                            ...
                            if (confidence<=true_thre)
                                ...
                                name=name.erase( name.length()-4, name.length()-1);
                                hi_name="Hi,"+name;
                                putText(result_cnn, hi_name, cv::Point(5, 60), cv::FONT_HERSHEY_SIMPLEX,0.75, cv::Scalar(0, 0, 255),2);
            ...
            // imshow("image", result_cnn);
            // waitKey(1);
            // cv::imwrite("image.jpg",result_cnn);
